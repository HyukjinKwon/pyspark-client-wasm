...
2. `SparkConnectGrpcException` import: API_CONTRACT section 1 says "from pyspark.errors", but in pyspark 4.0.0 it is at `pyspark.errors.exceptions.connect` (not re-exported). I import resiliently (try pyspark.errors, fall back to connect). Doc wording slightly off; no code change needed.
