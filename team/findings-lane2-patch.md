...
## Connection scheme (matches API_CONTRACT §2)
...
- **Lane 1 (`transport.GrpcWebStub`)** — default stub factory calls:
  ...
  §1 with `fn(request, *, metadata=None, timeout=None)`. **OPEN with lane 1.**
...
- None to API_CONTRACT.md §2. **Note for lanes 1/5**: `transport=grpcweb` is
